...
def __button1pressed(event):
  global __b1down
  global __bShowDiffDist
  __b1down = True
  __bShowDiffDist = False
  __mouseEventLock.acquire()
  __mouseEvents.append(("<Button-1>", mousePos()))
  __mouseEventLock.release()
  print("%d, %d, \\" % (mouseX(), mouseY()))  # print canvas coordinates on left mouse-click
...
def __button2pressed(event):
  ...
  global __bShowDiffDist
  __b2down = True
  __bShowDiffDist = True
  __mouseEventLock.acquire()
  __mouseEvents.append(("<Button-2>", mousePos()))
  print("Current distance from (%d, %d)" % (mouseX(), mouseY()))
  __mouseEventLock.release()
  __canvas.bind('<Motion>', findDiffDistance)  # capture hovering mouse in the window
...
# find the difference of distance between current point and initial point
def findDiffDistance(event):
    if showfindDiffDistance():
        _, clickedLocation = __mouseEvents[-1]  # get the location where middle-mouse has been clicked
        x_0, y_0 = clickedLocation  # set clicked location as initial coordinates
        # Compute the difference between two points and print
        stdout.write("\r" + "dx = %d, dy = %d \t" % (mouseX() - x_0, mouseY() - y_0))
        stdout.flush()
        sleep(1)

'''Return true if middle button is pressed lastly
   Return false if left and right button is pressed lastly
   Based on the result, distance between clicked point and hovered point will be calculated and printed'''
def showfindDiffDistance():
    return __bShowDiffDist